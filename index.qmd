...
map = {
 ...
 const circles = svg.append('g').selectAll('circle');
  ...
  .attr("r", 5)
    ...
    .append('title').text(d => `${d.cx} ${d.cy} ${d} ${d.cord}`)


  const proj = svg.append('g')
    ...
    })).concat(raw_points.filter((d) => (d.type == 'project' & d.year == 2023))
    .map((d) => ({
        name: d.name,
  ...
  function boxingForce() {
    ...
    const sim = d3.forceSimulation(conf_data_w_xy);
      sim.force('many', d3.forceManyBody().strength(1).distanceMax(10))
        .force("collide", d3.forceCollide().radius(5).strength(0.1).iterations(10))
  ...
  function color(point){
    ...
    function redraw() {
        ...
        svg.selectAll("circle")
                .attr("cx", d => d.x)
                .attr("cy", d => d.y)
                .append('title').text((d) => `${d.name} - ${d.y}`)
